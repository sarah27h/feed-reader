/* feedreader.js
 *
 * This is the spec file that Jasmine will read and contains
 * all of the tests that will be run against your application.
 */

/* We're placing all of our tests within the $() function,
 * since some of these tests may require DOM elements. We want
 * to ensure they don't run until the DOM is ready.
 */
$(function() {
    /* This is our first test suite - a test suite just contains
    * a related set of tests. This suite is all about the RSS
    * feeds definitions, the allFeeds variable in our application.
    */
    describe('RSS Feeds', function() {
        /* This is our first test - it tests to make sure that the
         * allFeeds variable has been defined and that it is not
         * empty. Experiment with this before you get started on
         * the rest of this project. What happens when you change
         * allFeeds in app.js to be an empty array and refresh the
         * page?
         */
        it('are defined', function() {
            expect(allFeeds).toBeDefined();
            expect(allFeeds.length).not.toBe(0);
        });

        /* This test loops through each feed
         * in the allFeeds object and ensures it has a URL defined
         * and that the URL is not empty and begin with http.
         */
        it('URL defined', function() {
            allFeeds.forEach(function(feed) {
                expect(feed.url).toBeDefined();
                expect(feed['url']).toMatch(/^http/);                
            });
        });

        /* This test loops through each feed
         * in the allFeeds object and ensures it has a name defined
         * and that the name is not empty and not have space only.
         */
        it('name defined', function() {
            allFeeds.forEach(function(feed) {
                expect(feed.name).toBeDefined();
                expect(feed.name).toMatch(/[\S]/);               
            });
        });
        
    });


    /* New test suite named "The menu" */
    describe('The menu', function() {
        const body = document.body,
            hidden = body.classList.contains('menu-hidden'),
            menuIcon = $('.menu-icon-link');

        /* This test ensures the menu element is
         * hidden by default. By checking that body element
         * class list contains 'menu-hidden' class or not
         */
        it('menu element is hidden by default', function() {
            expect(hidden).toBe(true);
        });

        /* This test ensures the menu changes
          * visibility when the menu icon is clicked. This test
          * should have two expectations: 
          * does the menu display when clicked -> 
          * body classList not contains 'menu-hidden' class
          *  and does it hide when clicked again ->
          * body classList contains 'menu-hidden' class
          */
        it('menu change visibility when menu clicked', function() {
            menuIcon.click();
            expect(body.classList.contains('menu-hidden')).toBe(false);
            menuIcon.click();
            expect(body.classList.contains('menu-hidden')).toBe(true);
        });

    });


    /* New test suite named "Initial Entries" */
    describe('Initial Entries', function() {
        const feed = document.querySelector('.feed');
        let testFeedEntryExist;
        /* This test ensures when the loadFeed
         * function is called and completes its work, there is at least
         * a single .entry element within the .feed container.
         * Remember, loadFeed() is asynchronous so this test will require
         * the use of Jasmine's beforeEach and asynchronous done() function.
         */

        // loadFeed() is asynchronous we put it inside beforeEach() 
        // to ensure it to load before expect
        beforeEach(function(done) {
            loadFeed(0, function(){
                done();
            });
        });

        it('a single .entry element within the .feed container', function(done) {
            // check if feed div contain childs using hasChildNodes()
            testFeedEntryExist = feed.hasChildNodes();
            console.log(testFeedEntryExist, feed);
            expect(testFeedEntryExist).toBe(true);
            done();
        });

    });


    /* New test suite named "New Feed Selection" */
    describe('New Feed Selection', function() {
        let firstFeed,
            firstFeedNew;
        const menuIcon = $('.menu-icon-link');
        /* This test ensures when a new feed is loaded
         * by the loadFeed function the content actually changes.
         * Remember, loadFeed() is asynchronous.
         */
       
        // loadFeed() is asynchronous we put it inside beforeEach() 
        // to ensure it to load before expect
        beforeEach(function(done) {
            firstFeed = document.querySelector('.entry').childNodes[1].innerText;
            loadFeed(2, function(){
                done();
            });
        });

        it('new feed is loaded -> the content actually changes', function(done) {
            firstFeedNew = document.querySelector('.entry').childNodes[1].innerText;
            console.log(firstFeed, firstFeedNew);
            expect(firstFeed).not.toEqual(firstFeedNew);
            done();
        });

    });
    
}());
